...
def split_data_own(dataset,
                   train_file=r'.\data\SuperCon3D\order_Tc_split\train_Tc.csv',
                   val_file=r'.\data\SuperCon3D\order_Tc_split\val_Tc.csv',
                   test_file=r'.\data\SuperCon3D\order_Tc_split\test_Tc.csv',
                   whole_file=r'.\data\order_data\targets.csv'):
    train_data = pd.read_csv(train_file)
    val_data = pd.read_csv(val_file)
    test_data = pd.read_csv(test_file)
    # whole_data = pd.read_csv(whole_file, header=None)
    whole_data = pd.read_csv(whole_file)
    ...
    # val_index = np.where(np.isin(whole_cf, val_cf) == True)[0]
    # test_index = np.where(np.isin(whole_cf, test_cf) == True)[0]
...
def OneHotDegree(data, max_degree=25, in_degree=False, cat=True):
